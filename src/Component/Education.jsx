        ...
        <section className=' w-full   items-center  lg:flex lg:flex-col'>
            {EDUCATION.map((education,index)=>(
                <section className='lg:mb-8 flex w-full  lg:w-[80%]  md:w-[90%] flex-wrap lg:items-center' key={index}>
                    <motion.section whileInView={{opacity:1,x:0}} initial={{opacity:0,x:-100}} transition={{duration:0.8,delay:1}} className='lg:flex-1 md:flex-1 block  w-full  '>
        <p className='mb-2 text-[10px] md:text-[16px] text-neutral-400 lg:text-xl tracking-wide font-semibold' >{education.year}</p>
            </motion.section>
            <motion.section whileInView={{opacity:1,x:0}} initial={{opacity:0,x:100}} transition={{duration:0.8,delay:1}} className='max-w-xl md:w-[40%] w-60%'>
